...
lift =3
lock=1.5
escapement = clock.BrocotEscapment(drop=drop, lift=lift, teeth=40, lock=lock)
...
#                 powered_wheel_module_increase=1, chain_wheel_pinion_thick_multiplier=2.25, pendulum_fixing=pendulumFixing, stack_away_from_powered_wheel=True)
...
#
# dial = clock.Dial(outside_d=180, bottom_fixing=False, top_fixing=True, style=clock.DialStyle.ARABIC_NUMBERS, font="Arial",inner_edge_style=clock.DialStyle.CONCENTRIC_CIRCLES, outer_edge_style=clock.DialStyle.RING)
# # dial = None
#
...
#                                  allow_bottom_pillar_height_reduction=False, endshake=1.5, second_hand=True, centred_second_hand=True)#, escapement_on_front=True)
#
#
...
#                     length=dial.get_hand_length(), thick=motionWorks.minute_hand_slot_height, outline=1, outline_same_as_body=False, chunky=True, second_hand_centred=True)
...
#
#
# assembly = clock.Assembly(plates, hands=hands, timeSeconds=30, pulley = pulley, pendulum=pendulum)#, timeHours=12, timeMins=0)#weights=[clock.Weight(height=245,diameter=55)]
#
# # show_object(plates.getPlate(back=True))
# # show_object(assembly.getClock(with_key=False, with_pendulum=True))
#
# assembly.show_clock(show_object, motion_works_colours=[clock.Colour.RED], bob_colours=[clock.Colour.PURPLE], plate_colour=clock.Colour.LIGHTGREY,
