...
train.gen_gears(module_size=1, module_reduction=moduleReduction, thick=2, thickness_reduction=0.9, chain_wheel_thick=4, useNyloc=False, pinion_thick_multiplier=3, style=gearStyle, powered_wheel_module_increase=1, chain_wheel_pinion_thick_multiplier=2)#,ratchetInset=True)#, chainModuleIncrease=1.1)
...
hands = clock.Hands(style=clock.HandStyle.SWORD, minute_fixing="square", minute_fixing_d1=motionWorks.get_minute_hand_square_size(), hourfixing_d=motionWorks.get_hour_hand_hole_d(), length=110, thick=motionWorks.minute_hand_slot_height, outline=1, outline_same_as_body=False, second_length=25)
...
assembly = clock.Assembly(plates, hands=hands, timeMins=0, timeSeconds=30, pulley = pulley, pendulum=pendulum)
assembly.printInfo()
